...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">商品发货管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>商品发货列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">商品发货列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										发货单号
									</label>
									<input type="text" id="fahuodanhaoSearch" class="form-control form-control-sm" placeholder="" aria-controls="tableId">
									<label>
										用户名
									</label>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('fahuodanhao')">发货单号<i id="fahuodanhaoIcon" class="fa fa-sort"></i></th>
											...
											<th onclick="sort('chengse')">成色<i id="chengseIcon" class="fa fa-sort"></i></th>
											...
											<th onclick="sort('shouhuodizhi')">收货地址<i id="shouhuodizhiIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('shangjiahao')">商家号<i id="shangjiahaoIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('fahuoriqi')">发货日期<i id="fahuoriqiIcon" class="fa fa-sort"></i></th>
											<th>操作</th>
										</tr>
									</thead>
									<tbody>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#fahuodanhaoSearch').val() != null && $('#fahuodanhaoSearch').val() != ''){
				searchForm.fahuodanhao ="%" +  $('#fahuodanhaoSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("shangpinfahuo/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
				fahuodanhao : searchForm.fahuodanhao,	
		...
		function setDataRow(item,number){
			...
			var fahuodanhaoCell  = document.createElement('td');

						 fahuodanhaoCell.innerHTML = item.fahuodanhao;


			row.appendChild(fahuodanhaoCell);
			...
			var chengseCell  = document.createElement('td');

						 chengseCell.innerHTML = item.chengse;


			row.appendChild(chengseCell);
			...
			var shouhuodizhiCell  = document.createElement('td');

						 shouhuodizhiCell.innerHTML = item.shouhuodizhi;


			row.appendChild(shouhuodizhiCell);
			...
			var fahuoriqiCell  = document.createElement('td');

						  fahuoriqiCell.innerHTML = item.fahuoriqi;


			row.appendChild(fahuoriqiCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("shangpinfahuo/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"shangpinfahuo");
